...
# ==============================================================================
	...
	def __HunterTranslations(self, data):
		...
		NOTA: Usa ~ come separatore perche' cmdchat tronca dopo lo spazio
		"""
		try:
			import dbg
			# Log dati ricevuti (primi 100 caratteri)
			preview = data[:100] if len(data) > 100 else data
			dbg.TraceError("[HUNTER LANG] __HunterTranslations ricevuto: " + preview + "...")
			
			# Formato: "category~key1=val1|key2=val2|..." (usa ~ invece di spazio)
			sep_idx = data.find("~")
			if sep_idx > 0:
				category = data[:sep_idx]
				trans_data = data[sep_idx + 1:]

				import hunter_translations
				hunter_translations.OnReceiveTranslations(category, trans_data)
				dbg.TraceError("[HUNTER LANG] Batch " + category + " processato OK")
			else:
				dbg.TraceError("[HUNTER LANG] ERRORE: formato dati non valido (manca ~ separatore) - data: " + data)
		except Exception as e:
			import dbg
	...
	def __HunterTranslationsReady(self, lang_code):
		...
		Usa sistema locale - niente dipendenza dal server!
		"""
		try:
			import dbg
			dbg.TraceError("[HUNTER LANG] __HunterTranslationsReady: " + str(lang_code))
			
			# Imposta la lingua localmente
			import hunter_translations
			hunter_translations.SetLanguage(str(lang_code))
			
			# Aggiorna UI se aperta
			try:
				if self.interface and hasattr(self.interface, 'wndHunterLevel') and self.interface.wndHunterLevel:
					if hasattr(self.interface.wndHunterLevel, 'OnTranslationsReady'):
	...
	def __HunterLanguageChanged(self, langCode):
		...
		Usa il sistema locale - niente piu' dipendenza dal server!
		"""
		try:
			import dbg
			dbg.TraceError("[HUNTER LANG] __HunterLanguageChanged: " + str(langCode))
			
			# Imposta la lingua localmente (le traduzioni sono in hunter_translations.py)
			import hunter_translations
			hunter_translations.SetLanguage(str(langCode))
			
			# Aggiorna la UI se visibile
			try:
				if self.interface and hasattr(self.interface, 'wndHunterLevel') and self.interface.wndHunterLevel:
					if hasattr(self.interface.wndHunterLevel, 'UpdateLanguageButton'):
