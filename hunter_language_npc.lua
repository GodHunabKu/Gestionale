...
quest hunter_language_npc begin
    state start begin
        ...
        when 20100.click begin
            ...
            -- IMPORTANTE: Resetta COMPLETAMENTE la cache traduzioni lato server
            -- 1. Resetta flag di caricamento per questo player
            if _G.hunter_translations_loaded then
                _G.hunter_translations_loaded[pid] = nil
            end
            
            -- 2. CRITICO: Ricarica la cache traduzioni per la nuova lingua
            -- Questo forza il server a usare i testi nella nuova lingua
            hg_lib.reload_translations_for_lang(selected_lang)

            -- 3. Invia le nuove traduzioni al client (force_reload = true)
            hg_lib.send_translations_to_client(selected_lang, true)
