...
class HunterLevelWindow(ui.ScriptWindow):
    ...
    def __UpdateHeaderContent(self):
        ...
        self.__HText(T("UI_SPENDABLE", "Spendibili:"), 250, 40, t["text_muted"])
        ...
        if nextKey:
            ...
            self.__HText(T("UI_NEXT", "Prossimo:"), 400, 63, t["text_muted"])
            self.__HText(nextTheme["name"], 460, 63, nextTheme["accent"])
        else:
            self.__HText(T("UI_MAX_RANK", "RANK MASSIMO!"), 400, 63, GOLD_COLOR)
    ...
    def __OnClickLangButton(self):
        """Mostra popup per selezionare la lingua"""
        try:
            # Se il popup esiste gia', toggle visibilita'
            if self.langPopup and self.langPopup.IsShow():
                self.langPopup.Hide()
                return
            
            # Crea popup se non esiste
            if not self.langPopup:
                self.__CreateLanguagePopup()
            ...
            import dbg
            dbg.TraceError("__OnClickLangButton error: " + str(e))

    def __CreateLanguagePopup(self):
        """Crea il popup di selezione lingua"""
        try:
            import hunter_translations
            
            # Ottieni lingue disponibili
            languages = hunter_translations.GetAvailableLanguages()
            
            # Dimensioni popup
            popupWidth = 120
            popupHeight = 30 + len(languages) * 28
            
            # Crea il contenitore del popup
            self.langPopup = ui.Bar()
            self.langPopup.SetParent(self.baseWindow)
            self.langPopup.SetPosition(WINDOW_WIDTH - 130, WINDOW_HEIGHT - popupHeight - 45)
            self.langPopup.SetSize(popupWidth, popupHeight)
            self.langPopup.SetColor(0xEE1A1A2E)
            self.langPopup.Show()
            
            # Bordo popup
            border = ui.Bar()
            border.SetParent(self.langPopup)
            border.SetPosition(0, 0)
            border.SetSize(popupWidth, 2)
            border.SetColor(0xFF00CCFF)
            border.AddFlag("not_pick")
            border.Show()
            self.langPopupButtons.append(border)
            ...
            for lang in languages:
                ...
                btn.SetText(langName)
                
                # Evidenzia lingua corrente
                if langCode == currentLang:
                    btn.SetText("* " + langName + " *")
                
                # Usa lambda con default argument per catturare il valore
                btn.SetEvent(lambda code=langCode: self.__OnSelectLanguage(code))
                btn.Show()
            ...
            import dbg
            dbg.TraceError("__CreateLanguagePopup error: " + str(e))
    ...
    def __OnSelectLanguage(self, langCode):
        ...
        try:
            ...
            # Cambia lingua (salva anche localmente e notifica server)
            if hunter_translations.ChangeLanguage(langCode):
                # Aggiorna bottone
                self.UpdateLanguageButton(langCode)
                
                # Nascondi popup
                if self.langPopup:
                    self.langPopup.Hide()
                
                # Distruggi popup per ricrearlo con la nuova selezione
            ...
            import dbg
            dbg.TraceError("__OnSelectLanguage error: " + str(e))

    def __DestroyLanguagePopup(self):
        """Distrugge il popup lingua"""
        try:
            for btn in self.langPopupButtons:
                if btn:
                    btn.Hide()
            self.langPopupButtons = []
            if self.langPopup:
    ...
    def OnTranslationsReady(self):
        ...
        try:
            import dbg
            dbg.TraceError("[HUNTER UI] OnTranslationsReady - Aggiornamento interfaccia...")
            
            # Aggiorna la lingua corrente dal modulo traduzioni
            import hunter_translations
            self.currentLang = hunter_translations.GetCurrentLanguage()
            
            # Aggiorna il bottone lingua (sempre)
            self.UpdateLanguageButton(self.currentLang)
            
            # Se la finestra non e' caricata, esci
            if not self.isLoaded:
                return
            
            # Aggiorna i nomi dei tab (sempre, anche se non visibile)
            self.__RefreshTabNames()
            
            # Se visibile, ricarica il contenuto del tab corrente
            if self.IsShow():
                if hasattr(self, 'currentTab'):
                    self.__LoadTabContent(self.currentTab)
                dbg.TraceError("[HUNTER UI] Interfaccia aggiornata per lingua: " + str(self.currentLang))
        except Exception as e:
            import dbg
            dbg.TraceError("[HUNTER UI] OnTranslationsReady errore: " + str(e))
    ...
    def __TranslateAchievementName(self, name):
        """Traduce i nomi degli achievement dal server"""
        # Mappa nomi italiani -> chiavi traduzione
        ACHIEV_MAP = {
            "Novizio (kill)": "ACHIEV_NOVICE_KILL",
            "Principiante (kill)": "ACHIEV_BEGINNER_KILL",
            "Cacciatore D (kill)": "ACHIEV_HUNTER_D_KILL",
            "Cacciatore C (kill)": "ACHIEV_HUNTER_C_KILL",
            "Cacciatore B (kill)": "ACHIEV_HUNTER_B_KILL",
            "Cacciatore A (kill)": "ACHIEV_HUNTER_A_KILL",
            "Elite S (kill)": "ACHIEV_ELITE_S_KILL",
            "Monarca (kill)": "ACHIEV_MONARCH_KILL",
        }
        clean_name = name.replace("+", " ")
        if clean_name in ACHIEV_MAP:
            return T(ACHIEV_MAP[clean_name], clean_name)
        return clean_name
    ...
    def __LoadAchievements(self):
        ...
        for a in self.achievementsData:
            ...
            achName = self.__TranslateAchievementName(a.get("name", ""))
            self.__CText(achName, 15, y + 3, col)
    ...
    def __LoadEvents(self, skipRequest=False):
        ...
        if not self.eventsData:
            ...
            if len(self.eventsData) > 8:
                extra_count = len(self.eventsData) - 8
                more_text = T("MORE_EVENTS", "... e altri %d eventi (clicca 'Apri Eventi')")
                try:
                    more_text = more_text % extra_count
                except:
                    more_text = "... e altri " + str(extra_count) + " eventi"
                self.__CText(more_text, 100, y, t["text_muted"])
                y += 19
    ...
    def Open(self):
        ...
        # IMPORTANTE: Richiedi le traduzioni dal server se non sono ancora caricate
        import hunter_translations
        if not hunter_translations.IsReady():
            hunter_translations.RequestTranslations()
        
        # Aggiorna la lingua corrente dal modulo traduzioni
        # Questo assicura che l'interfaccia usi le traduzioni corrette
        newLang = hunter_translations.GetCurrentLanguage()
        needsRefresh = (newLang != self.currentLang)
        
        if needsRefresh:
            self.currentLang = newLang
            self.UpdateLanguageButton(newLang)
            # FORZA ricostruzione completa dell'interfaccia con nuove traduzioni
            self.__BuildInterface()
        else:
            # Solo aggiorna tema se necessario (cambio rank)
            self.__UpdateTheme()
    ...
    def StartFractureDefense(self, fractureName, duration, color):
        ...
        minutes = duration / 60
        seconds = duration % 60
        msg = T("DEFENSE_START_MESSAGE", "[DIFESA FRATTURA] {NAME} - Difendi per {MINUTES}:{SECONDS:02d}!", {"NAME": fractureName, "MINUTES": minutes, "SECONDS": seconds})
