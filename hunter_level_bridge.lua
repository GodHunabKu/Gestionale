...
when chat."/hunter_request_trial_data" begin
        ...
        when login with pc.get_level() >= 5 begin
            ...
            -- Ottieni la lingua corrente del player
            ...
            -- Notifica che le traduzioni sono pronte (gestite lato client Python)
            cmdchat("HunterTranslationsReady " .. player_lang)
        ...
        when 16060.click or 16061.click or 16062.click or 16063.click or 16064.click or 16065.click or 16066.click begin
            ...
            if should_destroy == 1 then
                ...
                syschat("|cffFF0000[HUNTER SYSTEM]|r " .. hg_lib.get_text("fracture_destroyed", nil, "Questa frattura e' stata distrutta!"))
                syschat("|cffFF6600[HUNTER SYSTEM]|r " .. hg_lib.get_text("fracture_failed_defense", nil, "Un altro Hunter ha fallito la difesa."))
                syschat("|cffFFFF00[HUNTER SYSTEM]|r " .. hg_lib.get_text("fracture_seek_other", nil, "Cerca un'altra frattura. Buona caccia!"))
            ...
            if owner_pid > 0 then
                ...
                if time_diff > timeout then
                    ...
                    syschat("|cff00FF00[SISTEMA]|r " .. hg_lib.get_text("fracture_ownership_expired", nil, "Il diritto di conquista precedente e' scaduto. La frattura e' libera!"))
                elseif owner_pid == pid then
                    ...
                    syschat("|cffFF0000[SISTEMA]|r " .. hg_lib.get_text("fracture_conquered_other", nil, "Frattura conquistata da un altro Hunter!"))
                    syschat("|cffFF0000[SISTEMA]|r " .. hg_lib.get_text("fracture_available_in", {SECONDS = left}, "Tornera' disponibile tra " .. left .. " secondi se non viene aperta."))
                    return
                end
            end
            ...
            if reserved_leader > 0 then
                ...
                if get_time() - reserved_time > reservation_timeout then
                    ...
                    if not is_in_reserved_party then
                        ...
                        syschat("|cffFF0000[SISTEMA]|r " .. hg_lib.get_text("fracture_reserved_other_party", nil, "Frattura riservata da un altro party!"))
                        syschat("|cffFF0000[SISTEMA]|r " .. hg_lib.get_text("fracture_available_in", {SECONDS = remaining}, "Tornera' disponibile tra " .. remaining .. " secondi."))
                        return
                    end
                end
            end
            ...
            if party.is_party() then
                ...
                if first_touch_time == 0 then
                    ...
                    syschat("|cff00FF00[SISTEMA]|r " .. hg_lib.get_text("fracture_reserved_self", nil, "Frattura RISERVATA al tuo party per 60 secondi!"))
                    party.syschat("[HUNTER] " .. hg_lib.get_text("fracture_reserved_msg", nil, "Frattura riservata! Tutti devono toccarla entro 60 secondi!"))
                ...
                if get_time() - first_touch_time > touch_timeout then
                    syschat("|cffFF0000[SISTEMA]|r " .. hg_lib.get_text("fracture_timeout_lost", nil, "Tempo scaduto! Il party ha perso la prenotazione."))
                    party.syschat("[HUNTER] " .. hg_lib.get_text("fracture_timeout_freed", nil, "Tempo scaduto! La frattura e' tornata libera."))
        ...
        when hq_defense_timer.timer begin
            ...
            if elapsed >= duration then
                ...
                if killed >= total and total > 0 then
                    ...
                    local fail_msg = hg_lib.get_text("defense_timeout_failed", {COUNT = remaining_mobs}, "TEMPO SCADUTO! Mob rimasti: " .. remaining_mobs)
                    hg_lib.fail_defense(fail_msg)
                end
            end
        end
        ...
        -- ============================================================
        -- MULTI-LANGUAGE SYSTEM (Semplificato - Traduzioni in Python)
        -- ============================================================

        -- Richiesta lista lingue disponibili (opzionale)
        when chat."/hunter_request_languages" begin
            hg_lib.send_available_languages()
        end

        -- Salva lingua dal popup Python (le traduzioni sono gestite lato client)
